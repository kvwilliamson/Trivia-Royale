...
class TriviaGame:
    ...
    def select_categories(self):
        """Allow teams to select categories for trivia questions."""
        self.clear_screen()
        ...
        tk.Label(self.root, text="Each Team Select 3 Categories or click default:", font=FONTS["medium"], fg=COLORS["soft_coral"], bg=COLORS["light_blue"]).pack(pady=5)

        category_frame = tk.Frame(self.root, bg=COLORS["light_blue"])
        ...
        for i, category in enumerate(CATEGORIES):
            ...
            checkbox = tk.Checkbutton(
                category_frame, text=category, variable=var, font=FONTS["small"],
                fg=COLORS["soft_yellow"], bg=COLORS["light_blue"], selectcolor=COLORS["light_blue"], anchor="w"
            )
            checkbox.grid(row=i // 4, column=i % 4, sticky="w", padx=10, pady=2)
            if i >= len(CATEGORIES) - 3:  # Last three checkboxes can be modified
                checkbox.bind('<Double-Button-1>', lambda e, idx=i: self.modify_other_label(e, idx))
            self.checkbox_vars.append(var)
            self.checkbox_widgets.append(checkbox)

        error_label = tk.Label(self.root, text="", font=FONTS["small"], fg="red", bg=COLORS["light_blue"])
        ...
        def submit_categories():
            selected_categories = [self.checkbox_widgets[i].cget("text") for i, var in enumerate(self.checkbox_vars) if var.get()]
            if len(selected_categories) != 3 * self.num_teams:
                error_label.config(text=f"You have selected {len(selected_categories)} categories. You need to select exactly {3 * self.num_teams} categories.")
                return
            self.generate_prompt(selected_categories)

        tk.Button(button_frame, text="Use Selections", font=FONTS["small"], fg=COLORS["light_blue"], bg=COLORS["dark_teal"], command=submit_categories).pack(side=tk.BOTTOM, pady=10)
        tk.Button(button_frame, text="Use Default", font=FONTS["small"], fg=COLORS["light_blue"], bg=COLORS["dark_teal"], command=lambda: self.generate_prompt(["default"])).pack(side=tk.BOTTOM)

    def modify_other_label(self, event, checkbox_index):
        """Modify the label of selected 'Other' categories."""
        checkbox = self.checkbox_widgets[checkbox_index]
        current_text = checkbox.cget("text")

        input_box = tk.Entry(self.root, font=FONTS["small"], fg=COLORS["light_blue"], bg=COLORS["soft_yellow"])
        input_box.insert(0, current_text)
        input_box.place(x=event.x_root - self.root.winfo_rootx(), y=event.y_root - self.root.winfo_rooty())
        input_box.focus_set()

        def save_label(event=None):
            new_label = input_box.get()
    ...
    def select_difficulty(self):
        """Display the difficulty selection screen with three checkboxes."""
        self.clear_screen()
        ...
        # Dictionary to store the BooleanVar for each difficulty option
        self.diff_vars = {
            "Easy": tk.BooleanVar(),
        ...
        # Create checkboxes for each difficulty option
        tk.Checkbutton(
            diff_frame, 
            text="Easy", 
            variable=self.diff_vars["Easy"], 
            font=FONTS["small"],
            fg=COLORS["soft_yellow"],
            bg=COLORS["light_blue"]
        ...
        tk.Checkbutton(
            diff_frame, 
            text="Medium", 
            variable=self.diff_vars["Medium"], 
            font=FONTS["small"],
            fg=COLORS["soft_yellow"],
            bg=COLORS["light_blue"]
        ...
        tk.Button(
            ...
            bg=COLORS["dark_teal"],
            command=self.after_difficulty_selection  # Placeholder function
        ).pack(pady=20)

    def after_difficulty_selection(self):
        """Placeholder function after difficulty selection.
        Here you can process the selected difficulties or move to the next step.
        """
        # For now, just print out the selected difficulties
        selected = [level for level, var in self.diff_vars.items() if var.get()]
        print("Selected difficulty levels:", selected)
        
        # You can now continue to the next part of your game flow.
        # For example, you might call:
        # self.generate_prompt(selected_categories)

    def generate_prompt(self, selected_categories):
        """Generate a prompt for trivia questions based on selected categories."""
        if selected_categories == ["default"]:
            prompt = "default"
        else:
            categories_list = ", ".join(selected_categories)
            prompt = f"Generate 100 trivia questions randomly chosen from these topics: {categories_list}. Balance hard and medium questions, including some with wordplay or lateral thinking. Include interconnections for engagement. Avoid common knowledge. Ensure fact-checked answers. Format the results as only one question and answer per line, separated by |: Question? | Answer. No other formatting or grouping."

        questions = self.load_questions(prompt)
        if questions:
    ...
    def load_questions(self, prompt):
        """Load trivia questions either from a file or by using an external service via clipboard."""
        questions = []
        if prompt == "default":
            try:
                with open('c:/Users/Owner/trivia_questions_default(1).txt', 'r', encoding='utf-8') as file:
                    for line in file:
                        question, answer = line.strip().split('|')
                        questions.append({"question": question, "answer": answer})
                random.shuffle(questions)       
            except FileNotFoundError:
                print("Error: The questions file was not found.")
        else:
            # Open web browser with Deepseek or similar
            webbrowser.open("www.deepseek.com")
            
            # Copy prompt to clipboard for user to paste into the browser or text file
            pyperclip.copy(prompt)
            
            # Wait for user to manually save questions
            self.wait_for_user()

            # Now read from the manually saved file
            try:
                with open('c:/Users/Owner/Downloads/trivia_questions.txt', 'r') as file:
                    for line in file:
                        question, answer = line.strip().split('|')
                        questions.append({"question": question, "answer": answer})
                random.shuffle(questions)
            except FileNotFoundError:
                print("Error: The questions file was not found after manual input.")
        
        if not questions:
            print("No questions loaded. Using default or minimal set.")
            # Here you might want to load default questions or add some minimal questions
            questions = self.load_questions("default") if prompt != "default" else [{"question": "What?", "answer": "This"}]
    ...
    def play_theme(self):
        ...
        music_file = f"c:/Users/Owner/TQ_music_{random_number}.mp3"
        pygame.mixer.music.load(music_file)
        pygame.mixer.music.play(-1, 0.0)

    def play_intro_theme(self):
        """Play the intro theme for the game."""
        music_file = "c:/Users/Owner/TriviaRoyaleTheme.mp3"
        pygame.mixer.music.load(music_file)
    ...
    def play_winner_music(self):
        ...
        music_file = "c:/Users/Owner/TriviaChampion.mp3"
        pygame.mixer.music.load(music_file)
        pygame.mixer.music.play()
